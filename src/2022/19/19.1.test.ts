...
import { Blueprint, ClayRobot, GeodeRobot, ObsidianRobot, OreRobot } from "./blueprint.js";
...
describe("19.1", () => {
    ...
    describe("Robot factory", () => {
        ...
        it("With 3 ore, the factory can only produce clay robots of cost 3 as ore robots cost 4", () => {
            const blueprint = new Blueprint("Blueprint 3: Each ore robot costs 4 ore. Each clay robot costs 3 ore. Each obsidian robot costs 4 ore and 8 clay. Each geode robot costs 2 ore and 8 obsidian.")
    

            const availableRobots = blueprint.availableRobots(new Resources({ore:3, clay:7, obsidian:7}));
        ...
        it("With enough ore, the factory can produce any kind of robots", () => {
            ...
            const availableRobots = blueprint.availableRobots(new Resources({ore:3, clay:16, obsidian:9}));
        ...
        it("With an ore output you can build ore and clay robots", () => {
            ...
            const candidateRobots = blueprint.candidateRobots(new Resources({ore:1}));
        ...
        it("With an ore and clay output you can build ore,  clay and obsidian robots", () => {
            ...
            const candidateRobots = blueprint.candidateRobots(new Resources({ore:1, clay:1}));

            expect(candidateRobots.length).toEqual(3);
            expect(candidateRobots).toEqual([new OreRobot(3), new ClayRobot(3), new ObsidianRobot(3, 16)]);
        });
        ...
        it("With an ore and clay and obsidian output you can build ore, clay, obsidian and geode robots", () => {
            ...
            const candidateRobots = blueprint.candidateRobots(new Resources({ore:1, clay:1, obsidian:1}));
...
describe("Robot Optimizer", () => {
    describe("Ore Node", () => {
        it("Ore node returns the built resources and resources next turn on all ore kinds", () => {
            ...
            const resourcesExpected = new Resources({ore:1, clay:2, obsidian:1});
            expect(resourcesBuilt).toEqual(resourcesExpected);
    
            const resourcesNextTurn = node.resourcesNextTurn()
            const resourcesNextTurnExpected = new Resources({ore:1, clay:5, obsidian:1, geode:1});
        ...
        it("Ore node can build next turn ore node, and some time later a clay node, return both nodes with and without the robot built with 1 unit less of time", () => {
            ...
            expect(sucRobot.resources).toEqual(new Resources({ore:1, clay:1}))
        ...
        it("Ore node with that was able to build clay node and now it is able to build ore node returns ore node as successor and building no ore as successors", () => {
            ...
            const sucRobot = successors[0];
            expect(sucRobot.timeLeft).toEqual(22);
            expect(sucRobot.resources).toEqual(new Resources({ore:2}))
            expect(sucRobot.builtRobots.length).toEqual(2)
            expect(sucRobot.builtRobots).toEqual([new OreRobot(4), new OreRobot(4)])
        });
